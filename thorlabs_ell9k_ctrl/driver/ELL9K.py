...
# not to thread, but just to provide lock context management
...
class ELL9K:
    ...
    def __init__(self, port, verbose=False, *args, **kwargs):
        ...
        try:
            self._ser = serial.Serial(
                self._port, baudrate=9600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=8)
    ...
    def _parse_response(self, resp: str):
        try:
            ...
            print(
                "Response did not have right format ADDR [1 byte] - TAG [2 bytes] - DATA [>1 bytes]")
    ...
    def change_filter_name(self, idx: int, new_name: str):
        ...
        if (idx >= 0 and idx < self._pos_idx_max):
            ...
            # this will update the current filter if it is renamed
            self._current_filter = self._filter_list[self._current_pos_idx]
            self._ui_console = "Successfully changed " + \
                old_name + " to " + self._filter_list[idx]
        else:
